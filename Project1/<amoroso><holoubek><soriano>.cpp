...
public:
    BreakAlgorithm(istream& ifs, vector<int>& cipherDomain, vector<int>& cipherText) : da(28, vector<int>(28,0)), 
                    dc(cipherDomain.size() + 1, 
                    vector<int>(cipherDomain.size() + 1,0)), 
                    dp(28, vector<int>(28,0)),
                    cipherDomain(cipherDomain), 
                    cipherText(cipherText), 
                    output("output.txt"), 
                    sizeC(cipherDomain.size()), sizeA(0)
    {
        initDA(ifs);
        initDC();
        outerHillClimb();
    ...
    /*
        Permutates through possibly keyspace
    */
    ...
    void outerHillClimb() {
        ...
        char trg1;
        char trg2;
        //  For more precise decryption increase the constant to 25 for i and j
        for(int i = 1; i < 14; ++i) {
            cout << i << "th cycle" << endl;
    ...
    */
    int randomInitialKey(map<char,int>& primeFA) {
        ...
        //  for better score increase i to range of [20, 45]
        for(int i = 0; i < 4; ++i){
    ...
    */
    int innerHillClimb(){
        ...
        //  for better results increase i and j to 105
        for(int i = 1; i < 16; ++i) {
...
private:
    ...
    vector<pair<int,char>>& initialKey(map<char,int>& fa) {
        ...
        for(it = fa.begin(); it != fa.end(); ++it){
                float freq = (float)(it->second) / (float)(sizeC);
                int sentinel = freq * sizeC;   
                for(unsigned int i = 0; i < sentinel; ++i) {
                    if(!temp.empty()){
...
int main() {
    ...
    string input;
    cout << "Enter name of the input file w/o postfix .txt\n";
    cin >> input;
    ifstream afs(input + ".txt");
    if(!afs) {
        cerr << "couldn't open the file.. abort the program";
        exit(1);
    }
    vector<int> cipherText;
    vector<int> cipherDomain;
